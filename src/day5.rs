...
impl FromStr for Command {
    ...
    fn from_str(line: &str) -> Result<Self, Self::Err> {
        ...
        let groups = COMMAND_REGEX
            .captures(line)
            .ok_or(format!("Invalid command line: {}", line))?;
        Ok(Command {
...
fn calculate(input: &(Vec<Vec<char>>, Vec<Command>), reversed: bool) -> String {
    ...
    (0..stacks.len())
        .map(|stack_ix| {
            let pos = commands.iter().fold(
                Position {
                    stack_ix,
                    char_ix: 0,
                },
                |pos, command| pos.trace_back_command(command, reversed),
            );
